...
--
-- Host: localhost:3306
-- Waktu pembuatan: 22 Des 2024 pada 03.07
-- Versi server: 8.0.30
-- Versi PHP: 8.3.7
...
-- Table for certification tasks (managed by admin)
CREATE TABLE `certification_tasks` (
    `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
    `course_id` bigint UNSIGNED NOT NULL,
    `title` varchar(255) NOT NULL,
    `description` text,
    `task_file_url` varchar(255), -- URL to the task file uploaded by admin
...
-- Table for student certification submissions and status
CREATE TABLE `student_certifications` (
    `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
    `user_id` bigint UNSIGNED NOT NULL,
    `task_id` bigint UNSIGNED NOT NULL,
    `project_file_url` varchar(255), -- URL to student's project submission
    `certificate_file_url` varchar(255), -- URL to final certificate PDF
    `status` enum('pending', 'submitted', 'reviewing', 'passed', 'failed') NOT NULL DEFAULT 'pending',
    ...
    FOREIGN KEY (`task_id`) REFERENCES `certification_tasks` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
